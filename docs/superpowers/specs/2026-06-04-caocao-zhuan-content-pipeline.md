...
## 0.5 内容存储架构（A→B · 已与 James 定稿）

**剧情 / 对话 / 关卡 / 武将等一切内容 = 文件化的声明式数据，由引擎解释；既不硬编码进前端 UI 代码，也不上数据库。** UI 只负责"怎么显示"，内容是"数据"。

| 方案 | 内容 | 取舍 |
|---|---|---|
| **A · 现状** | JS 数据模块（`data/chapters/chNN/*.{map,story}.js`、`generals.js`…，`export const STORY/MAP/GENERALS = {...}`）。`scenarioRunner` 解释 `steps`，引擎/内容分离。纯静态 Web 游戏、无后端、存档走 `localStorage`。 | 最简单、零构建、随代码进 git；写内容要碰一点 JS |
| **B · 目标（内容铺开后平滑迁移）** | 纯 JSON / 结构化数据 + 加载器 + `data-validate` 校验器（§2）；文案集中（§5）为 i18n 留路；为将来**可选的剧本可视化编辑器**留单一数据面。 | 非程序也能写剧情；可校验、可工具化 |
| **C · 不做** | 数据库 / CMS / 后端。 | 单机单线剧情游戏无运行时编辑 / UGC / 多人内容协作需求 → 上 DB 需配后端 / 服务器 / 部署 = 过度工程。见 ops-index 决定关闭项 C13。 |

**迁移路径 A→B（引擎侧无感）**：先保持 A 可跑；逐章交付时新内容按 §1 schema 写成**纯数据对象**（JS 模块导出纯对象即可，仍零构建），`data-validate` 全程把关引用完整性与可达性；待"可视化剧本编辑 / 非程序协作"需求真正出现，再加纯 JSON 加载器与编辑器——引擎吃的是同一份 schema，A 与 B 对引擎透明。**任何阶段都不引入数据库。**
