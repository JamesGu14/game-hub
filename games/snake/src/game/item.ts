...
export type ItemType = 'shield' | 'magnet' | 'freeBoost' | 'foodRain';

export interface Item {
  pos: Vec2;
  type: ItemType;
  /** Rotation for visual animation. */
  rotation: number;
}
...
const ALL_TYPES: ItemType[] = ['shield', 'magnet', 'freeBoost', 'foodRain'];
...
export class ItemManager {
  ...
  fill(count: number = CONFIG.ITEM_COUNT): void {
    while (this.items.length < count) this.items.push(this.spawnOne());
  }
  ...
  update(dt: number): void {
    for (const it of this.items) it.rotation += dt * 1.2;
  }

  /** Try pickup. Applies effect on hit and returns the type, or null. */
  tryPickup(snake: Snake): ItemType | null {
...
export function applyEffect(snake: Snake, type: ItemType): void {
  switch (type) {
    case 'shield':
    ...
    case 'magnet':
      ...
      break;
    case 'freeBoost':
      snake.freeBoostTimer = Math.max(
        snake.freeBoostTimer,
        CONFIG.ITEM_FREE_BOOST_SEC,
      );
      break;
    case 'foodRain':
      snake.foodRainTimer = Math.max(
        snake.foodRainTimer,
        CONFIG.ITEM_FOOD_RAIN_SEC,
      );
      break;
  }
}
...
export function countActiveEffects(snake: Snake): number {
  let n = 0;
  if (snake.hasShield) n++;
  if (snake.hasMagnet) n++;
  if (snake.hasFreeBoost) n++;
  if (snake.hasFoodRain) n++;
  return n;
}
