...
import { type Vec2, angleDiff, clamp } from '../util/math';
...
export class Snake {
  ...
  get hasShield(): boolean { return this.shieldTimer > 0; }
  get hasMagnet(): boolean { return this.magnetTimer > 0; }
  get hasFreeBoost(): boolean { return this.freeBoostTimer > 0; }
  get hasFoodRain(): boolean { return this.foodRainTimer > 0; }
  /** Positions of food dropped this tick by boost length-drain. Consumed externally. */
  pendingFoodDrops: Vec2[] = [];
  private boostLossAccumulator = 0;
  ...
  respawn(pos: Vec2, angle: number, length: number): void {
  ...
  getSegments(): Vec2[] {
    ...
    const segments: Vec2[] = [{ x: this.pos.x, y: this.pos.y }];
    ...
    for (let i = 1; i < this.trail.length && segments.length < this.length; i++) {
      ...
      while (
        accumulated + segLen >= nextTargetDist &&
