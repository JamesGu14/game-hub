...
import { TAU, randRange, type Vec2 } from './util/math';

// ───── DOM ─────
const canvasEl = document.getElementById('game');
const fpsEl = document.getElementById('fps');
if (!(canvasEl instanceof HTMLCanvasElement)) throw new Error('#game not a canvas');
if (!fpsEl) throw new Error('#fps not found');
...
const foodRainAccumulator = new Map<Snake, number>();
...
function randomDiskPos(maxRadius: number): Vec2 {
  const a = Math.random() * TAU;
  const r = Math.sqrt(Math.random()) * maxRadius;
  return { x: Math.cos(a) * r, y: Math.sin(a) * r };
}

function findSafeSpawn(others: readonly Snake[]): { pos: Vec2; angle: number } {
  ...
  for (let attempt = 0; attempt < 30; attempt++) {
    ...
    if (ok) return { pos, angle: randRange(0, TAU) };
  }
  return { pos: randomDiskPos(maxR), angle: randRange(0, TAU) };
}

const aiSnakes: Snake[] = [];
const aiBrains: AIBrain[] = [];
const aiRespawnTimers = new Map<Snake, number>();
...
const allSnakes = (): Snake[] => [player, ...aiSnakes];

// ───── Player state ─────
let playerKills = 0;
let deathScreen: { timer: number; killerName: string; lengthAtDeath: number } | null = null;
...
function resize(): void {
  ...
  boostBtn.setCenter(
    w - CONFIG.BOOST_BTN_OFFSET_RIGHT,
    h - CONFIG.BOOST_BTN_OFFSET_BOTTOM,
  );
  bulletBtn.setCenter(
    w - CONFIG.BULLET_BTN_OFFSET_RIGHT,
    h - CONFIG.BULLET_BTN_OFFSET_BOTTOM,
  );
}
resize();
...
// ───── Touch ─────
function routeTouchStart(touch: Touch): void {
  if (joystick.tryClaim(touch)) return;
  if (bulletBtn.tryClaim(touch)) return;
  if (boostBtn.tryClaim(touch)) return;
}
function handleTouchStart(e: TouchEvent): void {
  for (let i = 0; i < e.changedTouches.length; i++) routeTouchStart(e.changedTouches[i]);
  e.preventDefault();
}
function handleTouchMove(e: TouchEvent): void {
  for (let i = 0; i < e.changedTouches.length; i++) joystick.update(e.changedTouches[i]);
  e.preventDefault();
}
...
canvas.addEventListener('mousedown', (e) => {
  ...
  if (joystick.tryClaim(t)) return;
  if (bulletBtn.tryClaim(t)) return;
  boostBtn.tryClaim(t);
});
canvas.addEventListener('mousemove', (e) => {
  if (mouseDown) joystick.update(mouseToTouch(e));
});
function releaseAllMouse(): void {
  joystick.release(-1);
  boostBtn.release(-1);
  bulletBtn.release(-1);
}
canvas.addEventListener('mouseup', () => {
  if (mouseDown) releaseAllMouse();
  mouseDown = false;
});
...
function mouseToTouch(e: MouseEvent): Touch {
  return { identifier: -1, clientX: e.clientX, clientY: e.clientY } as Touch;
}

// ───── Kill / Respawn ─────
function killSnake(victim: Snake, killer: Snake | null): void {
...
function respawnAI(snake: Snake): void {
...
const loop = new GameLoop({
  update: (dt) => {
    ...
    hash.clear();
    for (const s of allSnakes()) if (s.alive) hash.indexSnake(s);

    // AI think
    for (const brain of aiBrains) {
      if (brain.snake.alive) brain.think(dt, foods.foods, hash);
    }
    ...
    // Movement
    for (const s of allSnakes()) s.update(dt);
    ...
    for (const s of allSnakes()) {
      if (!s.alive || !s.hasMagnet) continue;
    ...
    for (const s of allSnakes()) {
      if (s.pendingFoodDrops.length > 0) {
        for (const p of s.pendingFoodDrops) foods.addFoodAt(p);
  ...
  render: () => {
    ...
    for (const s of aiSnakes) drawSnake(ctx, s);
