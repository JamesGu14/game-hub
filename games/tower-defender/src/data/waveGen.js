...
// [2026-06-13 平衡] 特种兵数量折扣——共性:让半数阵容失效的兵种不能按步卒数量满编出场。
// heavy/tengjia(抗物理,有效HP≈步卒4-6倍)0.45;flyer(直线绕过迷宫,仅对空将可打,33只/路=防空环数秒内被冲穿)0.6;
// shaman(集群互奶,25只=治疗封顶下的不死团)0.5。实测把特种波压回「该波最重但可解」带内(tools/play-through.mjs 组合档)。
const SPECIAL_COUNT_W = { heavy: 0.45, tengjia: 0.45, flyer: 0.6, shaman: 0.5 };
...
export function genWaves(template, params, seed) {
  ...
  for (let i = 0; i < waveCount; i++) {
    ...
    // ②每波最多 1 路装甲，多路同滚（L28 w19 曾三路40只重甲齐发）降级为步卒、数量保留——
    // 兵海压力不变，断崖削平。飞兵不限路数（全图直线汇聚城堡，防空环天然总防线）。
    let armorSeen = false;
    for (const sp of spawns) {
      const w = SPECIAL_COUNT_W[sp.enemyType];
      if (!w) continue;
      const isArmor = sp.enemyType === 'heavy' || sp.enemyType === 'tengjia';
      if (isArmor && armorSeen) { sp.enemyType = tiers[0]; continue; }   // 降级保数量（tiers[0] 恒 footman，campaign 门禁保证）
      if (isArmor) armorSeen = true;
      sp.count = Math.max(2, Math.round(sp.count * w));
    }
