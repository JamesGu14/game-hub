...
// 六将 lore：生平简介（史实向、一年级能懂）+ 武将特性（招牌技白话）。
const HERO_LORE = {
...
const UPGRADE_TEXT = `升级（最高 L${BAL.MAX_TOWER_LEVEL}）：每级 伤害↑ 射程↑ 攻速↑；升到 L${BAL.SIGNATURE_LEVEL} 解锁招牌技，之后继续变强。`;
...
export function drawHeroCard(ctx, view, generalId, anchor) {
  ...
  const upLines = wrap(ctx, UPGRADE_TEXT, CW - PAD * 2);
  ...
  const port = assets.images['gen_' + generalId];
  if (port) {
