...
// —— L1-L3 硬坡 ×1.5（黄忠基准 [改进⑩] dmg9→7、range3.5→4.5）——
assert.equal(l1.dmg, 7, 'L1 基准 dmg');
assert.equal(l1.range, 4.5, 'L1 射程');
assert.ok(Math.abs(l1.interval - 0.7) < 1e-9, 'L1 间隔');
assert.ok(Math.abs(l2.dmg - 10.5) < 1e-9, 'L2 dmg 7×1.5');
assert.equal(l2.range, 5.0, 'L2 射程 +0.5');
assert.ok(Math.abs(l2.interval - 0.63) < 1e-9, 'L2 间隔 ×0.9');
assert.ok(Math.abs(l3.dmg - 15.75) < 1e-9, 'L3 dmg 7×1.5²');
assert.equal(l3.range, 5.5, 'L3 射程 +1.0');
assert.ok(Math.abs(l3.interval - 0.567) < 1e-9, 'L3 间隔 ×0.81');

// —— L4-L5 软坡 ×1.3 ——
assert.ok(Math.abs(l4.dmg - 15.75 * BAL.UPGRADE_DMG_MULT_SOFT) < 1e-9, 'L4 dmg = L3×SOFT');
assert.ok(Math.abs(l4.interval - 0.567 * BAL.UPGRADE_INTERVAL_MULT_SOFT) < 1e-9, 'L4 间隔 = L3×SOFT');
assert.equal(l4.range, 6.0, 'L4 射程 +1.5');
assert.ok(Math.abs(l5.dmg - 15.75 * BAL.UPGRADE_DMG_MULT_SOFT ** 2) < 1e-9, 'L5 dmg = L3×SOFT²');
assert.ok(Math.abs(l5.interval - 0.567 * BAL.UPGRADE_INTERVAL_MULT_SOFT ** 2) < 1e-9, 'L5 间隔 = L3×SOFT²');
assert.equal(l5.range, 6.5, 'L5 射程 +2.0');

// —— 防爆守护：L5/L3 DPS 比 ∈ [1.8, 2.5]（dmg 等比缩放,比值不受基准改动影响 ≈1.873）——
...
// —— [spec §8.1] 武力排序不变量：五虎单发严格 赵>关>马>张>黄（黄忠7 仍最低）——
