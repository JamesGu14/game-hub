...
// 暴击（黄忠 L3 百步穿杨）：×2.5 且无视护甲（基于 L3 基础 20.25）
const c = calcDamage({ level: 3 }, huang, teng, crit);
assert.ok(c.isCrit, '触发暴击');
assert.ok(Math.abs(c.dmg - 20.25 * 2.5) < 1e-9, '暴击=L3基础×2.5 无视护甲(50.625)');

// L3 不暴击 → 走抗性
const nc = calcDamage({ level: 3 }, huang, teng, noCrit);
assert.ok(!nc.isCrit && Math.abs(nc.dmg - 20.25 * 0.5) < 1e-9, 'L3 普通 ×0.5');
...
assert.ok(Math.abs(cHard.dmg - 20.25 * 2.5) < 1e-9, '暴击跳过 dmgTakenMult(仍 50.625)');
